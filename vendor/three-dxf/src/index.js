...
var renderer, camera, scene, canvasWidth, canvasHeight;
...
export function Viewer(data, parent, width, height, font) {
    ...
    var allNumbers = scene.children.filter((item) => item.type === "Mesh");
    for (let i = 0; i < allNumbers.length; i ++) {
        var box = new THREE.Box3().setFromObject( allNumbers[i]);
        allNumbers[i].size = box.getSize();
    }
    ...
    function getClosedNumbers(){
        var px = raycaster.ray.origin.x;
        var py = raycaster.ray.origin.y;

        var res = [];
        for (var i = 0; i<allNumbers.length; i++) {
            let cx = allNumbers[i].position.x + allNumbers[i].size.x /2;
            let cy = allNumbers[i].position.y + allNumbers[i].size.y /2;

            if (Math.abs(px - cx) < 5 && Math.abs(py - cy) < 3) {
                res.push(allNumbers[i]);
            }
        }
        return res;
    }
    ...
    this.onMouseOver = ( event ) =>  {
        ...
        var intersects = getClosedNumbers();
        let oldColor = 0xfff
        for ( var i = 0; i < intersects.length; i++ ) {

            oldColor = intersects[ i ].material.color
            intersects[ i ].material.color.set( 0xfdd835 );    
            $("#cad-view > canvas").css('cursor', 'pointer');
        }
        this.render()
        for ( var i = 0; i < intersects.length; i++ ) {
    ...
    this.onMouseClick = ( event ) =>  {
        ...
        // var intersects = raycaster.intersectObjects( scene.children.filter((item) => item.type === "Mesh") );

        var intersects = getClosedNumbers();
        for ( var i = 0; i < intersects.length; i++ ) {
            currentElement = intersects[ i ];
            intersects[ i ].material.color.set( 0xfdd835 );

            console.log('Number', intersects[i].geometry.parameters.text);
    ...
    this.resize = () => {
        // Create scene from dxf object (data)
        i, entity, obj, min_x, min_y, min_z, max_x, max_y, max_z;
        dims = {
            min: { x: false, y: false, z: false},
            max: { x: false, y: false, z: false}
        };
    ...
    function drawMtext(entity, data) {
        ...
        switch (entity.attachmentPoint) {
            case 1:
                // Top Left
                text.position.x = entity.x;
                text.position.y = entity.y - entity.height;
            break;
            case 2:
                // Top Center
                text.position.x = entity.x - textWidth/2;
                text.position.y = entity.y - entity.height;
            break;
            case 3:
                // Top Right
                text.position.x = entity.x - textWidth;
                text.position.y = entity.y - entity.height;
            break;
            ...
            case 4:
                // Middle Left
                text.position.x = entity.x;
                text.position.y = entity.y - entity.height/2;
            break;
            case 5:
                // Middle Center
                text.position.x = entity.x - textWidth/2;
    ...
    function drawLine(entity, data) {
        ...
        if (entity.type === "LINE") entity.vertices = [entity.start, entity.end];
    ...
    function drawArc(entity, data) {
        ...
        arc.position.x = entity.x;
        arc.position.y = entity.y;
        arc.position.z = entity.z;
    ...
    function drawPoint(entity, data) {
        ...
        geometry.vertices.push(new THREE.Vector3(entity.x, entity.y, entity.z));
    ...
    function drawBlock(entity, data) {
        //var block = data.blocks[entity.block]; // TODO: back to entity.name

        let block = data.blocks.filter( (block) => block.name == entity.block)
